...
public class CircularImageView extends ImageView {
    ...
    private ColorFilter markFilter;
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        paintSelector = new Paint();
        paintSelector.setAntiAlias(true);
    ...
    public void setMarkFilter(ColorFilter filter) {
    ...
    public void setSelectorStrokeColor(int selectorStrokeColor) {
        if (paintSelector != null) {
            paintSelector.setColor(selectorStrokeColor);
        }
        this.invalidate();
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if(hasSelector && isSelected) { // Draw the selector stroke & apply the selector filter, if applicable
            ...
            canvas.drawCircle(center + outerWidth, center + outerWidth, ((canvasSize - (outerWidth * 2)) / 2) + outerWidth - 4.0f, paintSelector);
        }
        else if(markFilter != null) {
            paint.setColorFilter(markFilter);
        }
            ...
            borderRectF.left =  outerWidth / 2;
            borderRectF.top = outerWidth / 2;
            borderRectF.right = canvasSize - outerWidth / 2;
            borderRectF.bottom = canvasSize - outerWidth / 2;
            canvas.drawArc(borderRectF, 360, 360, false, paintBorder);
        }
    ...
    private void updateShadow() {
        ...
        paintSelector.setShadowLayer(radius, shadowDx, shadowDy, shadowColor);
    }
     ...
     * the StrokeCircle upon drawing.
     */
    protected void updateBitmapShader() {
        if (image == null)
            return;
